...
class LostFoundPortal(CustomerPortal):
    ...
    def portal_new_lost_item(self, **post):
        ...
        if request.httprequest.method == 'POST':
            ...
            # Send Email Template
            template = request.env.ref('lost_found_dashboard.email_template_v2_lost_claim_created', raise_if_not_found=False)
            if template:
    ...
    def portal_new_found_item(self, **post):
        ...
        if request.httprequest.method == 'POST':
            ...
            # Send Email Template
            template = request.env.ref('lost_found_dashboard.email_template_v2_found_item_created', raise_if_not_found=False)
            if template:
